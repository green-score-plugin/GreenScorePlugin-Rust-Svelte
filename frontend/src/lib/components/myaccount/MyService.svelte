...
{#if showAssignModal}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
        <div class="bg-white rounded-xl p-6 max-w-sm w-full mx-4 shadow-xl" onclick={(e) => e.stopPropagation()}>
            <div class="text-center mb-6">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2">Assigner un membre</h3>
                <p class="text-gray-600 text-sm">
                    Sélectionnez un membre à ajouter au service <span class="font-bold">"{serviceToAssign?.nom}"</span>.
                </p>
            </div>
            ...
            >
                ...
                <div class="flex flex-col gap-1 text-left">
                    <label for="userId" class="text-sm font-medium text-gray-700">Membre</label>
                    <div class="relative">
                        <select
                            name="userId"
                            id="userId"
                            required
                            class="w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-gs-green-950 focus:border-gs-green-950 sm:text-sm rounded-lg border bg-white"
                        >
                            <option value="" disabled selected>Choisir un membre...</option>
                            {#each availableMembers as member}
                                <option value={member.id}>
                                    {member.prenom} {member.nom}
                                    {member.service_id ? '(Déplacer)' : ''}
                                </option>
                            {/each}
                        </select>
                ...
                <div class="flex gap-3 justify-center mt-4">
                    <button
                            ...
                            class="px-5 py-2.5 rounded-lg border border-gray-200 text-gray-700 font-medium hover:bg-gray-50 transition-colors w-full"
                    >
                        Annuler
                    </button>
                    <button
                            type="submit"
                            class="px-5 py-2.5 rounded-lg bg-gs-green-950 text-white font-medium hover:bg-gs-green-800 shadow-sm transition-colors w-full"
                    >
                        Valider
                    </button>
                </div>
...
{#if showDeleteModal}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
        <div class="bg-white rounded-lg p-6 max-w-sm w-full mx-4 shadow-lg text-center" onclick={(e) => e.stopPropagation()}>
            ...
            <p class="text-gray-600 mb-6">
                Voulez-vous vraiment supprimer le service <span class="font-bold">"{serviceToDelete?.nom}"</span> ?
                <br><br>
                Cette action supprimera également le lien entre ce service et les utilisateurs associés.
            </p>
